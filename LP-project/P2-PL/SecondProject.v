...
Inductive cstep : (com * result)  -> (com * result) -> Prop :=
  ...
  | CS_AssertFalse :
      <{ assert false }> / RError --> <{ skip }> / RError
...
Proof.
  (* TODO *)
  intros. 
  apply hoare_consequence_pre with (P' := (ap parity X = parity m)%assertion).
  - eapply hoare_consequence_post.
  -- simpl. eapply hoare_while. eapply hoare_consequence_pre.
  --- verify_assn. eapply hoare_consequence_post.
  ---- eapply hoare_choice'.
  ----- eapply hoare_asgn.
  ----- admit.
  ---- verify_assn. admit.
  --- unfold "->>". intros. admit.
  -- verify_assn. remember (st X) as x. admit.
  - verify_assn.
Admitted.
