...
(* FILL IN HERE *)
End ExAssertions.
(* assn1 -> For any state st, all values of X should be less than or equal to Y *)
(* assn2 -> For any state st, either X = 3 or X is less than or equall to Y *)
(* assn3 -> For any state st, Z times Z in less than or equall to X and the predecessor of
Z times the predecessor of Z is less than or equall to X *)
(** [] *)
...
*)
(* FILL IN HERE
(* 1) c is a program that given a state {True} transforms it into {X=5} *)
(* 2) for all m, there is a program c that takes the state {X=m} into {X=m+5} *)
    [] *)
...
(* 4) invalid *)
(* 5) invalid *)
(* 6) valid -> sempre que a premisa é falsa é verdadeira *)
(* 7) valid -> é partially valid *)
(* 8) valid *)
(* 9) valid -> é partially valid *)
    [] *)
...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c QTrue st st' Hceval Hpre.
  apply QTrue.
Qed.
...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c PFalse st st' Hceval Hpre.
  apply PFalse in Hpre.
  contradiction.
Qed.
